...
export class Graph<T1, T2> implements IGraph<T1, T2> {
  ...
  public getAdjEdges(edge: Edge<T1, T2>) {
    ...
    for (let i = 0; i < this._edges.length; i++) {
      if (
        (this._edges[i].source.id === edge.source.id ||
          this._edges[i].source.id === edge.target.id ||
          this._edges[i].target.id === edge.source.id ||
          this._edges[i].target.id === edge.target.id) &&
        this._edges[i].id !== edge.id
      ) {
